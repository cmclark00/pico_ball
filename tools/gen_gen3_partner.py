...
def main():
    ...
    # C header for the standalone firmware's on-device Gen 3 trade.
    cout = os.path.join(REPO, "firmware", "path-b-standalone", "baked_party_gen3.h")
    with open(cout, "w") as f:
        f.write("// AUTO-GENERATED by tools/gen_gen3_partner.py — Gen 3 throwaway partner\n"
                "// trading section (896 bytes) for the standalone on-device capture.\n"
                "#ifndef BAKED_PARTY_GEN3_H\n#define BAKED_PARTY_GEN3_H\n#include <stdint.h>\n\n"
                f"static const uint8_t baked_party_gen3[{len(section)}] = {{\n")
        for i in range(0, len(section), 16):
            f.write("  " + ",".join(str(b) for b in section[i:i+16]) + ",\n")
        f.write("};\n#endif\n")
    print("wrote", cout)
