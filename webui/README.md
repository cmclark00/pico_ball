...
moves (+PP), OT, **EXP**, and **nicknamed / trade-evolves** flags. Gen 2 adds
held item, shiny/gender, mail, and happiness. Gen 3 additionally shows the
**National Dex #**, **Nature** (with the stat it shifts), **Ability**, **EVs**,
**Secret ID**, and a **checksum-valid** flag. When you drop a capture's `.json`
(or the whole `vault/` folder), the card also shows the **capture date**.

> Gen 3 Ability names come from `pokedata_gen3_abilities.js`, generated by
> `tools/gen_gen3_abilities.py`. The ability *pairings* are taken from the trade
> engine's Gen-3 `abilities.bin` (so they're era-correct — e.g. Gengar shows
> Levitate, its Gen 3 ability, not the modern Cursed Body).
